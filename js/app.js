...
document.getElementById("search-btn").addEventListener("click", function () {
  if (searchField.value != "") {
    ...
    fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${searchField.value}&units=metric&appid=2b334becae44bdcd12f3c9bf920b53d9`
    )
      .then((res) => {
        hideSpinner();
        return res.json();
...
function loadTime() {
  ...
  fetch(
    `https://api.ipgeolocation.io/timezone?apiKey=bb457660a4c742e4878a63e0822723ec&location=${timePlace.innerText}`
  )
    .then((res) => res.json())
